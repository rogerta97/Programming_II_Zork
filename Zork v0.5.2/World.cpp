...
void world::CreateWorld(){
	...
	creatures.Push_Back(new creature("Sunyol", "a big head", WEIRD_BEDROOM)); //this is the player creature

	creatures[PLAYER]->current_room = 0;
	creatures[PLAYER]->item = -1;
	creatures[PLAYER]->coins = 150; 
	creatures[PLAYER]->special_card = -1;
	creatures[PLAYER]->hp = 300;
	...
	creatures.Push_Back(new creature("bomboclats","just testing", EMPTY_ROOM)); 
	creatures.Push_Back(new creature("bomboclats","just testing", STUDYING_ROOM));
	creatures.Push_Back(new creature("bomboclats","just testing", LONELY_BATHTUB));
	creatures.Push_Back(new creature("bomboclats","just testing", BLACK_HOLE));
	creatures.Push_Back(new creature("bomboclats","just testing", FINAL_BOSS_ROOM));

	for (int i = 1; i < CREATURES_NUMB - 1; i++){
		creatures[i]->coins = rand() % 50 + 150; 
		creatures[i]->hp = 70; 
		creatures[i]->attack = rand() % 7; 
		creatures[i]->defense = rand() % 5;		
	}
	...
	items[ITEM_SWORD]->attack = 10; items[ITEM_SWORD]->shield_plus = 10; items[ITEM_SWORD]->hp_plus = 0;
	items[ITEM_ATTACK_CARD]->attack = 5; items[ITEM_ATTACK_CARD]->shield_plus = 0; items[ITEM_ATTACK_CARD]->hp_plus = 0;
	items[ITEM_HAMMER]->attack = 20; items[ITEM_HAMMER]->shield_plus = 0; items[ITEM_HAMMER]->hp_plus = 10;
	items[ITEM_SHIELD]->attack = 0; items[ITEM_SHIELD]->shield_plus = 50; items[ITEM_SHIELD]->hp_plus = 25;
	items[ITEM_PEAK]->attack = 15; items[ITEM_PEAK]->shield_plus = 0; items[ITEM_PEAK]->hp_plus = 0;
	...
	items[ITEM_FLAMETHROWER]->attack = 30; items[ITEM_FLAMETHROWER]->shield_plus = 0; items[ITEM_FLAMETHROWER]->hp_plus = 0;
	items[ITEM_CANONBOUX]->attack = 25; items[ITEM_CANONBOUX]->shield_plus = 0; items[ITEM_CANONBOUX]->hp_plus = 0;
	items[ITEM_WATERWALL]->attack = 0; items[ITEM_WATERWALL]->shield_plus = 0; items[ITEM_WATERWALL]->hp_plus = 0;
	items[ITEM_TNT]->attack = 50; items[ITEM_TNT]->shield_plus = 0; items[ITEM_TNT]->hp_plus = 0;
	items[ITEM_DIPLOMA]->attack = 0; items[ITEM_DIPLOMA]->shield_plus = 0; items[ITEM_DIPLOMA]->hp_plus = 0;
...
void world::Check_input(){
	...
	do{	
		...
		if (command != "go" && command != "open" && command != "close" && command != "move" && command != "teleport" && command != "pick" && command != "drop" && command != "equip" && command != "unequip" && command != "look" && command != "get" && command != "press" && command != "hit" && command != "win" && command != "attack"){
			if (command == "quit"){
			...
			if (command == "teleport"){
				if (move == "BURNER"){
					creatures[PLAYER]->current_room = BURNER;
					goto Start; 
				}
				else if (move == "AQUARIUM"){
					creatures[PLAYER]->current_room = AQUARIUM;
					goto Start;
				}
				else if (move == "MINE"){
					creatures[PLAYER]->current_room = MINE;
					goto Start;
				}
				}
			if (command == "go")
				...
				Pick_Item(move, creatures[PLAYER]->current_room);
				action_done = true;
			}
			else if (command == "drop"){
				Drop_item(move, creatures[PLAYER]->current_room);
				action_done = true;
			}
			else if (command == "get"){
...
void world::Pick_Item(const String& move, int current_room){
	...
	for (int i = 0; i < ITEMS_NUMB && quit_for == false; i++){
		if (creatures[PLAYER]->prov_inv[i] == -1){
			if (Get_Item_Room(Get_Num(move)) == creatures[PLAYER]->current_room){
				switch (Get_Num(move)){
				case ITEM_GLASS:
					quit_for = true;
...
void world::Drop_item(const String& move, int current_room){
	int i = 0;
	while (i < MAX_OBJECTS){
		if (move == creatures[PLAYER]->item_name(creatures[PLAYER]->prov_inv[i])){
			items[creatures[PLAYER]->prov_inv[i]]->item_room = creatures[PLAYER]->current_room;
			items[creatures[PLAYER]->prov_inv[i]]->carried = false;
...
bool world::isin_inventory(const String&item_name){
	...
	bool isin = false;
	while (i < MAX_OBJECTS){
		if (creatures[PLAYER]->prov_inv[i] == Get_Num(item_name))
			isin = true;
		i++;
	}
	return isin;
...
void world::Equip_item(const String& item_name){
	...
	int i = 0;
	{
		if (creatures[PLAYER]->item != -1)
			...
			if (creatures[PLAYER]->item == -1 && item_name == "sword" || item_name == "hammer" || item_name == "peak" && isin_inventory(item_name)){
				creatures[PLAYER]->item = Get_Num(item_name);
				equiped = true;
				printf("You equipped the %s!\n\n", item_name);
				if (creatures[PLAYER]->item == ITEM_SWORD){
					creatures[PLAYER]->attack = 30;
					creatures[PLAYER]->hp += 10;
				}
				if (creatures[PLAYER]->item == ITEM_HAMMER){
					creatures[PLAYER]->attack = 40;
					creatures[PLAYER]->hp += 10;
				}
				if (creatures[PLAYER]->item == ITEM_PEAK){
					creatures[PLAYER]->attack = 27;
				}

			}
			if (creatures[PLAYER]->item == -1 && item_name == "AttackCard" || item_name == "DefenseCard" || item_name == "HPCard" && isin_inventory(item_name)){
				creatures[PLAYER]->item = Get_Num(item_name);
				equiped = true;
				printf("You equipped the %s!\n\n", item_name);
			...
			if (creatures[PLAYER]->defense == -1 && item_name == "shield"  && isin_inventory(item_name)){
...
void world::Enter_Battle(const String& creature){ 
	printf("You entered to a battle with %s!!\n\n", creature); 
	int it = 0, rotate;
	bool win = false; 
	char change_Weapon[5];
	char weapon_selected[15]; 
	bool player_turn = true; 
	switch (creatures[PLAYER]->current_room){
	case EMPTY_ROOM: 
		while (creatures[PLAYER]->hp >= 0 && creatures[FIRE_CREATURE]->hp >= 0){		
			if (player_turn){
				player_turn = false;
				creatures[PLAYER]->damage = creatures[PLAYER]->attack - creatures[FIRE_CREATURE]->defense;
				srand(time(NULL));
				rotate = rand() % 1;
				if (rotate == 1) creatures[PLAYER]->damage += 1;
				else creatures[PLAYER]->damage -= 1;
				if (creatures[PLAYER]->damage < 0)
					creatures[PLAYER]->damage = 0;
				creatures[FIRE_CREATURE]->hp -= creatures[PLAYER]->damage;
				printf("You hited %s and you did ", creatures[FIRE_CREATURE]->name);
				...
				player_turn = true;
				srand(time(NULL)); 
				creatures[FIRE_CREATURE]->attack = rand() % 15;
				creatures[FIRE_CREATURE]->damage = creatures[FIRE_CREATURE]->attack - creatures[PLAYER]->defense;
			
				if (creatures[FIRE_CREATURE]->damage < 0)
					creatures[FIRE_CREATURE]->damage = 0;
				creatures[PLAYER]->hp -= creatures[FIRE_CREATURE]->damage;
				printf("%s hited you and did ", creatures[FIRE_CREATURE]->name);
				printf("%d damage\n", creatures[FIRE_CREATURE]->damage);
				Sleep(1000);
			}
				if (it == 10){
					printf("\n10 turns spended! Wanna take something from your backpack? ");
					printf("Actual battle state: You: %d // %s: %d\n", creatures[PLAYER]->hp, creatures[FIRE_CREATURE]->name, creatures[FIRE_CREATURE]->hp);
					scanf_s("%s", &change_Weapon, 15);
					String change_str(change_Weapon);
					if (change_str == "yes"){
						Show_inventory();
						printf(">: ");
						scanf_s("%s", &weapon_selected, 15);
...
void world::Hit_Stuff(const String& move){
	...
	if (creatures[PLAYER]->current_room == EARTH_CORRIDOR && (move == "rock" || move == "rocks") && item_str == "peak" && isin_inventory("peak")){
		exit[14]->open = true;
